...
public class NFCManager {
  ...
  public required init(alertMessage: String) {
    let nfcManager =
      self.currentActivity?.getSystemService(name: ContextStatic.NFC_SERVICE) as? NfcManager
...
extension NFCManager: NFCManagerProtocol {
  ...
  func getStandardURL(url: URL, urlType: URLType) -> URL? {
    ...
    case .facetime:
      return URL(string: "duo:\(url.absoluteString)")

    case .shortcut:
      let encodedShortcutID = url.absoluteString.addingPercentEncoding(
  ...
  public func writeTag(_ message: NFCMessage) {
    ...
    if var uriRecord = message.uriRecord {
      if uriRecord.urlType != nil {
        uriRecord.url =
          getStandardURL(url: uriRecord.url, urlType: uriRecord.urlType!) ?? uriRecord.url
      }
    }
  ...
  private func enableNfcForegroundDispatch() {
    ...
    let nfcPendingIntent = PendingIntent.getBroadcast(
      context: self.currentActivity, requestCode: 0, intent: intent, flags: 0)
    self.adapter?.enableForegroundDispatch(
      activity: self.currentActivity, intent: nfcPendingIntent, filters: [], techLists: [])
  }
    ...
    if let textRecord = message.textRecord {
      if let lang = textRecord.locale.languageCode {
        ...
        let record = NdefRecord(
          tnf: NdefRecord.TNF_WELL_KNOWN, _type: NdefRecord.RTD_TEXT, id: [], payload: payload)
        records.append(record)
      }
    }
...
extension NFCURIRecord {
  ...
  static func parseWellKnown(_ record: NdefRecord) -> NFCURIRecord? {
    ...
    guard let prefix = URI_PREFIX_MAP[Int(uintArray[0])],
      ...
      let url = URL(string: prefix + urlStr)
    else {
      return nil
    }
...
extension NFCTextRecord {
  static func parse(_ record: NdefRecord?) -> NFCTextRecord? {
    guard let record = record,
      ...
      record.getType() == NdefRecord.RTD_TEXT
    else {
      return nil
    }
    ...
    guard let languageCode = String(bytes: uintArray[1..<languageCodeLength], encoding: .ascii),
      let text = String(
        bytes: uintArray[languageCodeLength + 1..<uintArray.count], encoding: textEncoding)
    else {
      return nil
    }
